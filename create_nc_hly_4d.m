function create_nc_hly_4d(data,time,lvl,lon1,lat1,outfile)
...
nccreate('tmp.nc','level','Dimensions',{'level' length(lvl)},'Format','netcdf4','FillValue',fillvalue);
ncwriteatt('tmp.nc', 'level', 'long_name', 'Pressure Level');
ncwriteatt('tmp.nc', 'level', 'units', 'hPa');
ncwriteatt('tmp.nc', 'level', '_CoordinateAxisType', 'Pressure');
...
nccreate('tmp.nc','data','datatype','single','Dimensions',{'lon' length(lon1) ...
    'lat' length(lat1) 'level' length(lvl) 'time' length(time)},'Format','netcdf4','FillValue',fillvalue);
ncwriteatt('tmp.nc', 'data', 'standard_name', 'Relative Humdity');
ncwriteatt('tmp.nc', 'data', 'long_name', 'Relative Humidity');
ncwriteatt('tmp.nc', 'data', 'units', '%');
